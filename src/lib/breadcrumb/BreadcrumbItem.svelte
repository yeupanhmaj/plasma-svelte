...
>
	{#if href && !active}
		<a {href} class="plasma-breadcrumb-link" onclick={handleClick}>
			{#if icon}
				<span class="plasma-breadcrumb-icon"> {@render icon()} </span>
			{/if}
			<span>
				{#if children}
					{@render children()}
	...
	{:else if !active}
		<button type="button" {disabled} class="plasma-breadcrumb-btn" onclick={handleClick}>
			{#if icon}
				<span class="plasma-breadcrumb-icon"> {@render icon()} </span>
			{/if}
			<span>
