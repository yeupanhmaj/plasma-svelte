<script lang="ts">
	...
	const resolvedVariant = $derived(variant === 'default' ? 'primary' : variant)
</script>

<div class="plasma-progress plasma-progress--{size} {customClass}" {...restProps}>
	{#if label || showValue}
	...
	>
		{#if !indeterminate}
			...
			></div>
		{:else}
			<div class="plasma-progress-indeterminate plasma-progress-fill--{resolvedVariant}"></div>
		{/if}
	</div>
</div>
